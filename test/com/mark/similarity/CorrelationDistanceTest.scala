...
class CorrelationDistanceTest extends FunSuite with ShouldMatchers{
  ...
  def assignments = Array(new Assignment("A1", 0))

  val dataSource = new GradeDataSource[GPAMark]() {
    ...
    def getGrade(
      ...
      Option(new GPAMark(map.get((student, grader)).get.toInt, 2000))
    }
  ...
  test("should produce correlation 0.7544237083454246 using data source"){
    val simpleLearner = new SimpleLearner[GPAMark]
    val predictorsMap = simpleLearner.train(dataSource)
    val corrDistance = new CorrelationDistance[GPAMark](dataSource)
